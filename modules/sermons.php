...
function get_cpt_sermon_meta( $post_id = null )
{
    ...
    */
    ...
    $info .= '<div id="letterhead-logo" class="print-only"><img src="/wp-content/themes/apostle/graphics/stc-letterhead.png" class="print-only" /></div>';
    ...
    */
    ...
    $info .= stc_post_title( $title_args );
...
function update_sermon_bbooks( $sermon_id = null )
{
    ...
    if ( $scripture_citations ) {
        foreach( $scripture_citations as $reading_id ) {
            ...
            if ( is_numeric($book) ) { $bbook_id = $book; } else if ( isset($book[0]) ) { $bbook_id = $book[0]; } else { $bbook_id = null; }
            ...
            if ( $reading_id = post_exists($txt) ) {
                ...
                if ( $book_id && $chapterverses ) {
                    ...
                    if ( !is_wp_error($post_id) ) {
                        ...
                        $info .= "reading postarr: <pre>".print_r($postarr,true)."</pre>";
