...
function get_snippets ( $args = array() ) {
	...
	foreach ( $snippets as $snippet_id ) {
		...
		if ( is_search() && $snippet_status == "active" ) {
			...
			$snippet_logic_info .= "snippet_display: ".$snippet_display;
			$snippet_logic_info .= "any_all: ".$any_all;
						...
						if ( $wildcard == false ) {
							...
							if ( !$matched_post && $slug ) {
								...
								foreach ( $p_types as $p_type ) {
									...
									// wip trying to figure out why kind find match for grain of wheat event series -- not working as it is below
