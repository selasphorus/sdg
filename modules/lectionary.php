...
function parse_date_str ( $args = array() ) {
	...
	foreach ( $calc_components as $component ) {
		...
		if ( preg_match('/[0-9]{4}-[0-9]{2}-[0-9]{2}/', $component) ) {
			...
			if ( empty($calc_basis)) {
				$calc_basis = trim(substr($date_calculation_str,strpos($date_calculation_str,$component)+strlen($component)));
	...
	if ( $calc_basis ) {
		...
		if ( $verbose == "true" ) { $info .= ">> get_calc_bases_from_str using str date_calculation_str: $date_calculation_str<br />"; }
