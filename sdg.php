...
function sdg_msg_bar( $args = array() ) {
    ...
    if ( !empty($banner_content) ) {
        ...
        // If the current post has no banner content, then look for a post to feature
        // Check to see if there is a webcast event currently livestreaming
        if ( !$postID && $prioritize_livestream && function_exists('get_live_webcast_id') ) {
            $postID = get_live_webcast_id();
        }
        // If no ID was submitted AND there's no livestreaming event, look for a featured post
        if ( !$postID ) {
            $postID = getFeaturedPostId( $post_type, $num_posts );
        }
    }
...
function getFeaturedPostId( $post_type = 'post', $num_posts = 1) {
    $postID = null;
    // Set up args to find featured post(s)
    $wp_args = array(
        'post_type'   => $post_type,
        'post_status' => 'publish',
